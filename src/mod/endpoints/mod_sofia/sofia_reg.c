...
#else
	switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_NOTICE, "Destroying registration handle for %s [%p]\n", gateway_ptr->name, (void*)gateway_ptr);
#endif
...
void sofia_reg_fire_custom_gateway_state_event(sofia_gateway_t *gateway, int status, const char *phrase)
{
	...
	if (switch_event_create_subclass(&s_event, SWITCH_EVENT_CUSTOM, MY_EVENT_GATEWAY_STATE) == SWITCH_STATUS_SUCCESS) {
		...
		if (!zstr(gateway->from_user)) {
			switch_event_add_header_string(s_event, SWITCH_STACK_BOTTOM, "Register-Username", gateway->from_user);
		}
	...
	for (gateway_ptr = profile->gateways; gateway_ptr; gateway_ptr = gateway_ptr->next) {
		if (gateway_ptr->deleted) {
			...
			if (gateway_ptr->state == REG_STATE_NOREG || gateway_ptr->state == REG_STATE_DOWN) {
				...
				switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_NOTICE, "Deleted gateway %s [%p] State=%d\n", gateway_ptr->name, (void*)gateway_ptr, gateway_ptr->state);
		...
		case REG_STATE_FAILED:
			{
				...
				if (gateway_ptr->max_retry_seconds > 30 && sec > gateway_ptr->max_retry_seconds)
				{
...
switch_status_t sofia_reg_add_gateway(sofia_profile_t *profile, const char *key, sofia_gateway_t *gateway)
{
	...
	if ((gp = switch_core_hash_find(mod_sofia_globals.gateway_hash, key))) {
		if (gp->deleted) {
			switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "Removing deleted gateway %s [%p] from hash.\n", gp->name, (void*)gp);
	...
	if (status == SWITCH_STATUS_SUCCESS) {
		switch_event_t *s_event;
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_NOTICE, "Added gateway '%s' [%p] to profile '%s'\n", gateway->name, (void*)gateway, gateway->profile->name);
